...
s = ttk.Style()
s.configure('TFrame', background='#646464')
main.title("Audit Processor SBT")
main.geometry("950x600")
frame = ttk.Frame(main, width=950, height=500, style='TFrame')
...
def make_query(struct):
    ...
    for i in range(len(output)):
        if 'REG_' in output[i]:
            ...
            if p.match(value):
                print('Correct Policy description:'+struct['description'])
                ...
                print('Wrong Policy description:' + struct['description'])
                print('Did not pass: ', struct['value_data'])
                print('Failed to load the value: ', value)
...
def check():
    ...
    frame2.config(highlightbackground="#ffe873")
    frame2.place(relx=0.535, rely=0.1, width=375, relwidth=0.4, relheight=0.8, anchor='n')
    listbox_succes = Listbox(frame2, bg="#e6ce00", font=app_font, fg="white", listvariable=vars1, selectmode=MULTIPLE,width=50, height=27, highlightthickness=3)
...
text = Text(frame, bg="#e6cfb1", fg="white", font=app_font, width=105, height=45, highlightthickness=3)
btn_font = Font(family="Courier New", size=10)
save_btn = Button(frame, bg="#4b8bbe", fg="white", font=btn_font, text="Save", width=13, height=1, command=save_config).place(relx=0.01, rely=0.01)
import_btn = Button(frame, bg="#4b8bbe", fg="white", font=btn_font, text="Import", width=13, height=1,command=import_audit).place(relx=0.01, rely=0.065)
download_btn = Button(frame, bg="#4b8bbe", fg="white", font=btn_font, text="Download", width=13, height=1,command=extract_file).place(relx=0.01, rely=0.12)
select_all_btn = Button(frame, bg="#4b8bbe", fg="white", font=btn_font, text="Select All", width=13, height=1,command=select_all).place(relx=0.01, rely=0.175)
deselect_all_btn = Button(frame, bg="#4b8bbe", fg="white", font=btn_font, text="Deselect All", width=13, height=1,command=deselect_all).place(relx=0.01, rely=0.229)
exit_btn = Button(frame, bg="#4b8bbe", fg="white", font=btn_font, text="Exit", width=13, height=1,command=main.quit).place(relx=0.01, rely=0.284)
global e
e = Entry(frame, bg="#ffffff", font=btn_font, width=25, textvariable=querry).place(relx=0.2, rely=0.04)
find_btn = Button(frame, bg="#4b8bbe", fg="white", font=btn_font, text="Find", width=10, height=1,command=find).place(relx=0.42, rely=0.035)
check_btn = Button(frame, bg="#4b8bbe", fg="white", font=btn_font, text="Check", width=10, height=1,command=check).place(relx=0.520, rely=0.035)
